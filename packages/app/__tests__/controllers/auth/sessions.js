...
describe("SESSION MANAGEMENT API", () => {
  beforeEach(() => {
    ...
    jest.spyOn(UserSessionService.prototype, "validateSession").mockResolvedValue(MOCK_USER_SESSIONS[0]);
  });
  ...
  describe("GET /api/auth/sessions", () => {
    it("200 - List active sessions", async () => {
      ...
      jest.spyOn(UserSessionService.prototype, "getActiveSessions").mockResolvedValue(activeSessions);
  ...
  describe("DELETE /api/auth/sessions/:sessionId", () => {
    it("400 - Cannot revoke current session", async () => {
      ...
      expect(response.body.message).toBe(Messages.CANNOT_REVOKE_CURRENT_SESSION);
    });

    it("404 - Session not found or unauthorized", async () => {
      jest.spyOn(UserSessionService.prototype, "revokeSession").mockResolvedValue(null);
    ...
    it("200 - Session revoked successfully", async () => {
      jest.spyOn(UserSessionService.prototype, "revokeSession").mockResolvedValue({ isActive: false });
  ...
  describe("POST /api/auth/signout/others", () => {
    it("200 - Revokes all other sessions", async () => {
      jest.spyOn(UserSessionService.prototype, "revokeOtherSessions").mockResolvedValue({ modifiedCount: 2 });
  ...
  describe("POST /api/auth/signout/all", () => {
    it("205 - Revokes all sessions and clears cookie", async () => {
      jest.spyOn(UserSessionService.prototype, "signoutAll").mockResolvedValue({ modifiedCount: 3 });
